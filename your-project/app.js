...
let EQUIPMENT = [
    // Accessories
    { id: "bards-cunning", name: "Bard's Cunning", slot: "Accessory", rarity: "legendary", set: "", iconic_group: "none" },
    ...
    { id: "dominion", name: "Dominion", slot: "Weapon", rarity: "legendary", set: "", iconic_group: "weapon" },
...
function renderSlots(setId){
  ...
  const makeDiamond = (slot)=>{
    ...
    // NEW: wrapper reserves sqrt(2)*size so rows don't overlap
    ...
    // click: set active slot + auto-filter library
    div.onclick = ()=>{
      ...
      const slotSel = el("#filter-slot");
      if (slotSel) slotSel.value = state.filters.slot;
      renderSlots(setId);
      renderLibrary();
    };
    ...
    if(it){
      div.innerHTML = `
        ...
        <div class="badges">
          <span class="badge ico">${it.iconic||1}</span>
          <span class="badge crit" style="background:${it.crit?'#22c55e':'#1b5e20'}">▲</span>
        </div>
      `;
      setTimeout(()=>{
        div.querySelector(".badge.ico").onclick = (e)=>{
          e.stopPropagation();
          it.iconic = (it.iconic||1) + 1; if(it.iconic>5) it.iconic=1;
          e.currentTarget.textContent = it.iconic;
          renderStats(setId);
        };
        div.querySelector(".badge.crit").onclick = (e)=>{
          e.stopPropagation();
          it.crit = !it.crit;
          e.currentTarget.style.background = it.crit ? "#22c55e" : "#1b5e20";
          renderStats(setId);
        };
      },0);
    } else {
      const ph = slotPlaceholder[slot] || slotPlaceholder[slot.split(" ")[0]] || "images/gear/weapon.webp";
      div.innerHTML = `<div class="inner"><img src="${ph}" alt=""></div>`;
    }

    wrap.appendChild(div);
    return wrap; // return wrapper instead of the diamond itself
  };
...
function renderLibrary(){
  ...
  filteredItems().forEach(it=>{
    ...
    tile.onclick=()=>{
      ...
      if(slot.startsWith("Accessory") && it.slot==="Accessory") equip(state.activeSet, slot, it);
      else if(slot===it.slot) equip(state.activeSet, slot, it);
    };
...
function setPieces(setId){
  ...
  for(const s of slotsList){ const it=items[s]; if(!it||!it.set) continue; counts[it.set]=(counts[it.set]||0)+1; }
  return counts;
}
...
function iconicCostFor(it){
  ...
  const tb=ICONIC_COST[grp]||{}; for(let L=2; L<=lvl; L++){ const row=tb[String(L)]; if(row) sum=addMats(sum,row); }
  return sum;
}
function applyCrit(stats,it){ if(!it.crit) return; for(const a of (it.attributes||[])) stats[a.stat]=(stats[a.stat]||0)+a.value*0.05; }
function applyIconic(stats,it){ const lv=(it.iconic||1)-1; if(lv<=0) return; for(const a of (it.attributes||[])) stats[a.stat]=(stats[a.stat]||0)+a.value*0.04*lv; }
...
function renderStats(setId){
  const {totals,matsAll,iconicAll} = aggregate(setId);
  const wanted=["Attack","Defense","Health","All Damage"];
  el(setId==="set1"?"#kpi1":"#kpi2").innerHTML =
    wanted.map(n=>`<div class="kpi"><div>${n}</div><div class="val">${fmt(totals[n]||0)}%</div></div>`).join("");
  const block=(obj)=>Object.keys(obj).length?Object.entries(obj).map(([k,v])=>`<div class="mat">${k}: <b>${v}</b></div>`).join(""):`<div class="mat" style="opacity:.6">None</div>`;
  el(setId==="set1"?"#mats1":"#mats2").innerHTML = block(matsAll);
  el(setId==="set1"?"#iconic1":"#iconic2").innerHTML = block(iconicAll);
}
...
document.querySelectorAll(".toggle").forEach(btn=>{
  btn.onclick=()=>{ const id=btn.dataset.target; const box=el("#"+id); box.classList.toggle("open"); btn.textContent = box.classList.contains("open")?"Hide overall stats":"Show overall stats"; };
});
...
el("#equipFile").onchange=async (e)=>{ const f=e.target.files[0]; if(!f) return; EQUIPMENT = normalize(JSON.parse(await f.text())); renderAll(); };
el("#setsFile").onchange=async (e)=>{ const f=e.target.files[0]; if(!f) return; SET_EFFECTS = JSON.parse(await f.text()); renderAll(); };
...
el("#compareBtn").onclick=()=>{
  ...
  body.innerHTML = `<div class="delta">${
    keys.map(k=>{
