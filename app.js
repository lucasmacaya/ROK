...
const FORMATIONS = [
    { name: "Wedge", bonus: { Attack: 0 } },
    { name: "Pincer", bonus: { Defense: 0 } },
    { name: "Delta", bonus: { Health: 0 } },
];
...
function roundUpToHalf(x) {
  return Math.ceil(x * 2) / 2;   // 2.1 -> 2.5, 3.4 -> 3.5, 3.6 -> 4.0
}

function applyCrit(stats, it) {
  if (!it.crit) return;
  for (const a of it.attributes || []) {
    // Crit = +30% of base line, rounded UP to the next 0.5
    const critBonus = roundUpToHalf(a.value * 0.3);
    stats[a.stat] = (stats[a.stat] || 0) + critBonus;
  }
}
